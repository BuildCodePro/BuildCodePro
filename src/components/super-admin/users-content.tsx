...
//         description="Manage company_owner admins, estimators, and engineers across all companies."
//       />
...
import { useAdminUsersQuery } from "@/services/adminService";
import type {
  AdminUserItem,
  AdminUserRole,
  AdminUsersStats,
...
function mapAdminUserToPlatformUser(item: AdminUserItem): PlatformUser {
  return {
    id: item.user_id,
    name: item.name,
    email: item.email,
...
function buildStats(stats?: AdminUsersStats): DashboardStat[] {
  if (!stats) return [];
...
export function UsersContent() {
  ...
  const usersQuery = useAdminUsersQuery({
    ...
    status:
      statusFilter !== "all" ? (statusFilter as AdminUserStatus) : undefined,
  });

  const stats = useMemo(
    () => buildStats(usersQuery.data?.stats),
    [usersQuery.data],
  );

  const users: PlatformUser[] = useMemo(() => {
    const items = usersQuery.data?.items ?? [];
    return items.map(mapAdminUserToPlatformUser);
  }, [usersQuery.data]);
