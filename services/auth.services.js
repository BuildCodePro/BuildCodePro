...
import { AUTH_LOG_TYPE_IDS, USER_STATUS_IDS } from '../constants/lookup.constants';
import { AppError } from '../errors';
import { createAccessToken, createOtpToken, sendEmail } from '../utils';
import { formatUserProfile } from '../utils/lookup.utils';
...
export class AuthService {
	...
	async login() {
		...
		await prisma.auth_log.create({
			data: {
				user_id: user.id,
				type_id: AUTH_LOG_TYPE_IDS.LOGIN,
			},
		});
	...
	async OtpVerify() {
		...
		if (type === 'reset') {
			...
			updateData = { status_id: USER_STATUS_IDS.ACTIVE, remember_token: null };
		}
